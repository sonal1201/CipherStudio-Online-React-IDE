// Convert the folder path into json
 ...
 */
export interface ReactFolder {
  folderName: string;
  items: (TemplateFile | ReactFolder)[];
}

/**
 * Type representing either a file or folder in the template structure
 */
export type TemplateItem = TemplateFile | ReactFolder;
 ...
 * @returns Promise resolving to a ReactFolder object
 */
...
export async function readTemplateStructureFromJson(
  ...
  try {
    ...
    return JSON.parse(data) as ReactFolder;
  } catch (error) {
    throw new Error(
